...
SSAPCmdResponseContext spim_send_command_data(uint8_t *data_buffer, uint32_t buffer_len, uint8_t data_command, uint32_t slice_len)
{
    ...
    while(!one_frame_end_flag) { /* one frame while start */
        ...
        *crc_value_buf = get_crc16_nbs(spim_send_buffer, actual_tranfer_len);
	    ...
	    //LOGD("send command:%d,sync_word:%d,content:%d,crc_value:%d\n",spim_song_command_context->command,spim_song_command_context->sync_word,spim_song_command_context->content,spim_song_command_context->crc_value);
		...
		LOGD("send data:%s",tx_buffer_point+point_location);
	...
	//LOGD("[UNLOCK]send_command_data_count=%d, data_command=%d\n\n", send_command_data_count, data_command);
	LOGD("pis_song_response_context->command:%d\n",spis_song_response_context.command);
	pthread_mutex_unlock(&spi_send_data_mutex);

	//printf("spis_song_response_context:%s",spis_song_response_context);
